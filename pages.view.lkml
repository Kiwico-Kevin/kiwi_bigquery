  ...
  dimension: path_type {
    sql:
      CASE
        ...
        WHEN ${path} LIKE '/customer/subscriptions%' THEN 'Sub Hub'
        WHEN ${path} LIKE '/subscription/manage/index/id/%' THEN 'Manage Subscription'
        WHEN ${path} LIKE '/customer/store-orders%' THEN 'Manage Store Orders'
        WHEN ${path} LIKE '/kiwicrate_customeraccount/credits%' THEN 'Rewards'
        WHEN ${path} LIKE '/customer/reset-password%' THEN 'Forgot Password'
        WHEN ${path} LIKE '/customer/login%' THEN 'Account Login'
